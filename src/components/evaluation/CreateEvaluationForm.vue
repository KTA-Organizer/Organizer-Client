    ...
    <v-dialog width="500" v-model="newEvaluation">
        <v-card>
            ...
            <v-card-text>
                ...
                <v-form ref="formPeriodName">
                    <v-text-field :rules="defaultRule" name="periodname" label="Naam van de periode" id="periodname" type="text" v-model="periodname" required></v-text-field>
                </v-form>
            </v-card-text>
            <v-card-actions>
...
export default {
    ...
    methods: {
        ...
        async createNewEvaluation() {
            if (this.$refs.formPeriodName.validate()) {
                const date = new Date(this.newEvaluationDate);
                const newEvaluationId = await this.$http.createNewEvaluation(this.student.id, this.module.id, date, this.periodname);
