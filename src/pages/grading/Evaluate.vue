...
<div>
    <v-layout row justify-center>
        ...
        <!-- TODO check if dialog works --> 
        <confirmdialog v-bind:model.sync="popup" v-on:confirm="deleteEval(popupId)" :name="popupName" :action="verwijderen"></confirmdialog>
    </v-layout>
    <v-layout row wrap>
    ...
    <v-layout row-wrap>
        <v-flex xs4 class="text-xs-left">
            <v-breadcrumbs>
                <v-breadcrumbs-item v-for="breadcrumb in breadcrumbs" :key="breadcrumb.id" :disabled="breadcrumb.disabled">
                    {{ breadcrumb.text }}
                </v-breadcrumbs-item>
            </v-breadcrumbs>
        </v-flex>
    ...
    <v-layout row-wrap>
        <v-flex xs12 sm4 offset-xs1>
            <v-select label="Selecteer een module" v-bind:items="modulesDropdown" v-model="selectedModuleName" persistent-hint @input="selectItem()"></v-select>
        </v-flex>
    </v-layout>
    <v-layout v-if="moduleSelected" row wrap class="text-xs-left">
        <v-flex offset-xs1>
            <div>
                <v-btn v-if="!newEvalTable" @click="newEval" color="primary">
                    <v-icon class="mr-2">add</v-icon>Nieuwe Evaluatiefiche
                </v-btn>
                <v-btn v-if="newEvalTable" @click="newEvalTable = false; updateEval = true" color="primary">
                    <v-icon class="mr-2">undo</v-icon>Back
                </v-btn>
                <v-btn v-if="newEvalTable && !updateEval" @click="makeJSON(false)" color="primary">
                    <v-icon class="mr-2">save</v-icon>Evaluatie Opslaan
                </v-btn>
                <v-btn v-if="newEvalTable && updateEval" @click="makeJSON(true)" color="primary">
                    <v-icon class="mr-2">save</v-icon>Evaluatie Opslaan
                </v-btn>
    ...
    <v-flex xs10 offset-xs1>
        <v-list two-line v-if="moduleSelected && !newEvalTable && gotEvals">
            ...
            <template v-for="evaluation in prevEvals[0].evaluaties.slice().reverse()">
                <v-divider :key="evaluation.id"></v-divider>
                <v-list-tile v-bind:key="evaluation.id" @click="getEvaluation(evaluation.id)">
                    ...
                    <v-btn color="error" v-on:click.stop="deleteEvalPopup(evaluation.id, evaluation.name)" class="right" dark>
                        ...
                        <v-flex>
                            <v-card color="gray darken-3" class="black--text text-xs-left" height="100%">
                                <v-container fluid grid-list-lg>
                                    <v-flex>
                                        <v-text-field name="EvalFicheName" label="Naam evaluatiefiche" v-model="evalName"></v-text-field>
                                    </v-flex>
                                </v-container>
                            </v-card>
                        </v-flex>
                                        ...
                                        <v-layout row-wrap v-for="(crit, k) in doel.criteria" :key="k">
                                            ...
                                            <v-flex xs8>
                                                <v-layout row-wrap v-for="(aspect, l) in crit.aspecten" :key="l">
                                                    ...
                                                    <v-flex xs2>
                                                        <v-btn class="evalCard black--text text-xs-left" style="height: 100%;margin: 0;" v-if="activeBoxesCreated" v-on:click="logYes(aspect.id)" :class="{green: activeBoxes['yes' + aspect.id]}">
                                                            <p>JA</p>
                                                        </v-btn>
                                                    </v-flex>
                                                    <v-flex xs2>
                                                        <v-btn class="evalCard black--text text-xs-left" style="height: 100%; margin: 0" v-if="activeBoxesCreated" v-on:click="logNo(aspect.id)" :class="{red: activeBoxes['no' + aspect.id]}">
                                                            <p>NEE</p>
                                                        </v-btn>
                                                    </v-flex>
                                                    <v-flex xs2>
                                                        <v-btn class="evalCard black--text text-xs-left" style="height: 100%; margin: 0" v-if="activeBoxesCreated" v-on:click="reset(aspect.id)">
                                                            <p>Reset</p>
                                                        </v-btn>
                                                    </v-flex>
                                                </v-layout>
        ...
        createActiveBoxes: function (module) {
            ...
            for (var i = 0; i < module.length; i++) {
                for (var j = 0; j < module[i].categorieen.length; j++) {
                    for (
                        var k = 0; k < module[i].categorieen[j].doelstellingen.length; k++
                    ) {
                        for (
                            var l = 0; l < module[i].categorieen[j].doelstellingen[k].criteria.length; l++
                        ) {
                            for (
                                var m = 0; m <
