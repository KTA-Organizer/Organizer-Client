<template>
  <v-layout column>
      ...
      <v-flex xs12 md6 v-else>
        <v-card>
          <v-card-text>
            ...
            <p
              v-if="isStudent"
            >Op dit platform kan je terecht voor het bekijken van je rapporten. Bij een nieuw rapport, zul je deze hier kunnen bekijken, printen of downloaden. Bij een nieuw rapport krijg je een e-mail.</p>
          </v-card-text>
        </v-card>
      </v-flex>
    ...
    <v-layout wrap row v-if="isStudent">
      <v-flex md3 xs12 v-for="report in studentReports" :key="report.id">
        <v-card>
          <v-card-title>
          ...
          <v-card-actions>
            <v-btn :to="'/rapport/'+report.id" dark hover color="primary">bekijken</v-btn>
          </v-card-actions>
        </v-card>
      </v-flex>
    </v-layout>
...
export default {
  ...
  async created() {
    if (this.isStudent) {
      ...
      this.studentReports = result.items.map(report =>
      ...
      this.meldingen = meldingen.map(m =>
        Object.assign(m, {
